...
contract('Memory Token', (accounts) => {
  ...
  describe('token distribution', async () => {
    ...
    it('mints tokens', async () => {
      await token.mint(accounts[0], 'http://www.token-uri.com/nft')
      
      //Checks for increase in total supply
      result = await token.totalSupply()
      assert.equal(result.toString(), '1', 'total supply is correct')

      //Checks for increase in owner balance
      result = await token.balanceOf(accounts[0])
      assert.equal(result.toString(), '1', 'balanceOf is correct')

      //Checks if token belongs to owner
      ...
      //Checks if owner can see all tokens
